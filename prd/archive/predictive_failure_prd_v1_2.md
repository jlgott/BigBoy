...
[Chunk + Vectorize]
  ...
  - Events: structured features (counts, co-occurrences, temporal)
       ↓
[Score]
...
[Store in single index with labels]
       ↓
[Inference]
  - New chunk → KNN → distance-weighted neighbor labels → score
       ↓
...
### 3.2 Core Concepts
- Everything becomes a chunk with a feature vector, scores, and metadata
...
- **Unified distance metric:** Euclidean distance on normalized numeric vectors for all data types
...
Event data is primarily structured (fault codes, maintenance types, event categories) with minimal rich free-text. Text embeddings would obscure the structural patterns that actually predict failures: which event types occur together, how frequently, and in what sequence.

Therefore, events use **structured feature vectors** with the same Euclidean distance approach as sensors and oil samples. This keeps the architecture uniform and preserves interpretability.
...
#### 4.3.3 Chunk Representation (Structured Features)

| Feature Category | Features | Description |
|------------------|----------|-------------|
| **Event Type Counts** | `count_fault_code_A`, `count_fault_code_B`, ... | Count of each event type in window |
| | `count_maint_type_X`, `count_maint_type_Y`, ... | Count of each maintenance type in window |
| **Temporal Features** | `days_since_last_{type}` | Days since most recent occurrence of each event type |
| | `total_events` | Total event count in window |
| | `event_density` | Events per day in window |
| | `trend_direction` | Increasing (+1), stable (0), or decreasing (-1) event rate |
| **Co-occurrence Features** | `cooccur_A_B`, `cooccur_A_C`, ... | Binary or count: did event types A and B both occur in window? |

**Feature Vector Construction:**

```
[count_fault_A, count_fault_B, ..., count_maint_X, ...,
 days_since_fault_A, days_since_fault_B, ...,
 total_events, event_density, trend_direction,
 cooccur_A_B, cooccur_A_C, ...]
```

The exact feature set depends on the event type vocabulary in the source system. Maintain a registry of known event types; unknown types are logged and queued for addition.

#### 4.3.4 Normalization
- Fleet baseline per feature per asset model (same approach as sensors)
- Trimmed mean/std to handle assets with anomalous event volumes
- `days_since_*` features: cap at window size (14 days) if event type never occurred

#### 4.3.5 Distance Metric
- Euclidean distance on normalized feature vector

**Why Euclidean over alternatives:**

| Metric | Problem for Events |
|--------|-------------------|
| Jaccard | Ignores counts; "1 fault" = "15 faults" |
| Cosine | Scale-invariant; high event volume assets should score differently |
| Euclidean | Preserves magnitude, works with continuous features, matches sensor/oil approach |

#### 4.3.6 Text Embedding (Optional Secondary Signal)

For the minority of events with rich free-text descriptions (>20 words), text embedding can provide supplementary signal:

- Computed only when `len(description) > threshold`
- Stored alongside structured features, not replacing them
- If used at inference: weighted combination of structured distance (primary) and text distance (secondary)
- **v1:** Structured features only. Text embedding parked for future validation.

#### 4.3.7 Interpretability

Structured features enable human-readable explanations:

> "This pattern matched because both windows had 3+ fault code X, 2+ fault code Y, and event density > 5/day"

This is not achievable with pure text embeddings.
...
| window_size | For sensors/events: chunk window size | Conditional |
| has_prior_window | For sensors: whether delta is meaningful | Conditional |
| feature_version | Version tag for feature extraction approach | Yes |
...
- Metadata stored alongside feature vectors for filtering
...
| feature_name | Feature name (signal, analyte, or event feature) |
...
### 7.4 Event Type Registry

Lookup table for event feature construction:

| Field | Description |
|-------|-------------|
| event_type_id | Internal identifier |
| event_type_name | Human-readable name |
| source_system | Origin system |
| feature_index | Position in feature vector |
| added_at | When type was registered |

New event types are logged and queued for registry addition (requires feature vector rebuild for historical data).
...
| Event pattern separation | Needs validation | Structured features, second priority |
...
| Feature | Notes |
|---------|-------|
| Text embedding for events | Only if rich descriptions prove valuable; structured features primary |
...
- Event feature vector requires known event type vocabulary; new types require rebuild
...
[Chunking Layer] -- Time windows (sensor), point-in-time (oil), rolling window (events)
       |
       v
[Feature Extraction Layer] -- Numeric vectors: stats, analytes, structured event features
       |
       v
[Normalization Layer] -- Fleet baseline normalization
       |
       v
...
[Storage Layer] -- Vector DB with metadata
       |
       v
[Inference Layer] -- KNN with Euclidean distance, distance-weighted scoring
       |
       v
...
## Appendix C: Data Type Summary

| Data Type | Chunking | Features | Normalization | Distance |
|-----------|----------|----------|---------------|----------|
| Sensor | 5-min windows | stats + delta (8-9 features) | Fleet baseline | Euclidean |
| Oil | Single sample | Analyte values (~15-20 features) | Fleet baseline | Euclidean |
| Events | 14-day rolling window | Counts + temporal + co-occurrence | Fleet baseline | Euclidean |

All data types follow the same pattern: structured numeric features → fleet normalization → Euclidean distance. This architectural consistency simplifies implementation, debugging, and explanation.
